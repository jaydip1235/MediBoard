...
function Profile() {
  ...
  const getUser = async () => {
      try {
          ...
          setPhone(data.phone)
          setEmail(data.email)
          setBloodG(data.bloodGroup)
          setParam1(data.param1)
          console.log(data.param1);
          setParam2(data.param2)
          setInfo(data.moreInfo)
          setUimg(data.image)
          setUpdatedAt((data.updatedAt).substr(0,10))
      } catch (err) {
          console.log(err)
          history.push('/error')
      }
          ...
          <div className="bg-white shadow rounded overflow-hidden">
            <div className="px-4 pt-0 pb-4 cover">
              <div className="media align-items-end profile-head bg-danger">
                <div className="profile mr-3"><img src={uimg} alt="..." width="130" className="rounded mb-2 img-thumbnail" /><div className="list-inline-item ml-2 text-center d-block">
            ...
            <div className="px-4 py-3">
              ...
              <div className="p-4 rounded shadow-sm bg-light userdet text-center">
                ...
                <p className="font-italic mb-0"><strong className="text-danger">Blood group: </strong>{bloodG?bloodG:"No data"}</p>
                <p className="font-italic mb-0"><strong className="text-danger">Blood pressure: </strong>{liParam1[(liParam1.length)-1]?liParam1[(liParam1.length)-1].val:"No data"}</p>
                <p className="font-italic mb-0"><strong className="text-danger">Blood sugar: </strong>{liParam2[(liParam2.length)-1]?liParam2[(liParam2.length)-1].val:"No data"}</p>
                <p className="font-italic mb-0"><strong className="text-danger">More info: </strong>{info ? info : "No data"}</p>
                <p className="font-italic mb-0"><strong className="text-danger">Last updated: </strong>{updatedAt?updatedAt:"No data"}</p>
              </div>
            </div>
