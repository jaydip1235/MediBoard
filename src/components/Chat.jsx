...
const Chat = props => {
  ...
  const handleMessageSubmit = message => {
    ...
    axios
      .post("http://localhost:3000/chatbot", data)
      .then(response => {
        ...
        if (resp.startsWith("Okay")) {
          ...
          const doctor=textsLi[7];
          const date=textsLi[9].substr(0,10);
          const time=textsLi[11];
          const config = {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("authToken")}`,
            },
          };
          axios.post(`/api/private/user/chat/${userID}/${doctor}/${date}/${time}`,config).then(history.push('/')).catch((err)=>{console.log(err)});
        }
      })
      .catch(error => {
        history.push('/error');
